#!/usr/bin/env python3
"""
QB Protocol - Siri Response Store
Cloud-backed response store for consistent Siri outputs.
"""
...
class SiriResponseStore:
    """Cloud-backed response store with local cache."""
    ...
    def get_response(self, intent: str, utterance: str = "") -> Dict[str, Any]:
        key = intent if intent in self.responses else "fallback"
        data = dict(self.responses.get(key, DEFAULT_RESPONSES["fallback"]))
